...
interface WalletModalProps {
  ...
  onFHEVMConnect: () => Promise<void>
  onWalletSelect?: (walletId: 'unisat' | 'leather' | 'xverse' | 'okx') => Promise<void>
}

export function WalletModal({ open, onOpenChange, onFHEVMConnect }: WalletModalProps) {
    ...
    return [
      {
        id: 'metamask',
        name: 'MetaMask',
        description: 'Connect to Zama fhEVM testnet',
        icon: '🦊',
      ...
      <DialogContent className="sm:max-w-md bg-surface-1 border-white/10">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            <Wallet className="w-5 h-5 text-yo-yellow" />
            <span className="text-white font-bold uppercase">
              Connect Wallet
            </span>
          </DialogTitle>
          <DialogDescription className="text-white/45">
            Connect your EVM wallet to fhEVM Network
          </DialogDescription>
        </DialogHeader>
          ...
          {wallets.map((wallet) => (
            <button
              ...
              className={`
                ...
                ${wallet.isInstalled
                  ? 'border-white/10 bg-surface-2 hover:bg-surface-3 hover:border-yo-yellow/50 cursor-pointer'
            ...
            >
              ...
              {!wallet.isInstalled ? (
                <ExternalLink className="w-4 h-4 text-white/40 group-hover:text-yo-yellow transition-colors" />
              ) : isConnecting && selectedWallet === wallet.id ? (
                <div className="w-5 h-5 border-2 border-yo-yellow border-t-transparent rounded-full animate-spin" />
              ) : (
                <div className="w-5 h-5 rounded-full border-2 border-white/20 group-hover:border-yo-yellow transition-colors" />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center justify-center gap-2 pt-2 pb-1">
          <div className="px-3 py-1 rounded-full bg-yo-yellow/10 border border-yo-yellow/20">
            <span className="text-xs text-yo-yellow font-bold uppercase tracking-[1.2px]">Testnet Mode</span>
          </div>
        </div>
